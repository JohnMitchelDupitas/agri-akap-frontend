    ...
    <table class="form-table">
      ...
      <tbody>
        <tr v-for="(row, i) in paddedRows" :key="i">
          ...
          <td>{{ row?.crop_type }}</td>
          <td>{{ row?.crop_stage }}</td>
          <td class="ta-center">{{ row?.area_planted }}</td>
          <td class="ta-center">{{ row?.area_damaged }}</td>
          <td class="ta-center">{{ row?.est_yield_loss_pct }}</td>
        </tr>
      </tbody>
    </table>

    <p class="totals">{{ rows.length }} affected farmer(s) · {{ totalDamaged }} ha damaged</p>
