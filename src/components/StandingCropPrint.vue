<template>
  <div id="standing-crop-print" class="brgy-doc" v-if="rows?.length">
    <div class="doc-header">
      <h1>Republic of the Philippines</h1>
      <h2>Province of Isabela</h2>
      <h2>Municipality of Echague</h2>
      <h2 class="brgy-line">BARANGAY {{ barangay || '____________' }}</h2>
      <h4 class="doc-title">STANDING CROP REPORT — {{ cropLabel.toUpperCase() }}</h4>
    </div>
    ...
    <table class="form-table">
      ...
      <tbody>
        <tr v-for="(row, i) in paddedRows" :key="i">
          ...
          <td>{{ row?.crop_type }}</td>
          <td>{{ row?.variety }}</td>
          <td class="ta-center">{{ row?.area_ha }}</td>
          <td>{{ row?.growth_stage }}</td>
          <td class="ta-center">{{ row?.est_harvest_date }}</td>
        </tr>
      </tbody>
    </table>

    <p class="totals">{{ rows.length }} farmer(s) · {{ totalHa }} ha</p>
...
const totalHa = computed(() =>
  props.rows.reduce((s, r) => s + (Number(r.area_ha) || 0), 0).toFixed(2),
);
